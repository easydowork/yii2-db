...
class Query extends BaseObject implements QueryInterface, ExpressionInterface
{
     ...
     */
    public function createCommand(Connection $db)
    {
    ...
    {
        return ClassHelpers::createObject([
...
PATTERN;
     ...
     * $query = (new \easydowork\db\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subquery = (new \easydowork\db\Query)->from('user')->where(['active' => true])
     * $query = (new \easydowork\db\Query)->from(['activeusers' => $subquery]);
     *
